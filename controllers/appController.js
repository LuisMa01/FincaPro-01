...
const getAllApps = asyncHandler(async (req, res) => {
  pool
    .query(
      "SELECT date_init, date_end, date_act_key, date_crop_key, date_id, date_user_key, crop_camp_key, crop_plant_key, crop_name, user_name, user_nombre, act_name	FROM public.table_app_date INNER JOIN public.table_crop ON crop_id = date_crop_key INNER JOIN public.table_activity ON act_id = date_act_key INNER JOIN public.table_user ON user_id = date_user_key ORDER BY date_id ASC;"
    )
    .then((results) => {
